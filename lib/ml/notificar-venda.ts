...
export async function carregarProdutos(tenantId: string) {
 ...
 */
export async function metaMargemAtual(tenantId: string): Promise<number | null> {
  try {
 ...
 */
export async function enviarEPersistirEntrega(tenantId: string, eventId: string, type: NotificationEventType, payload: SalePushPayload, isSummary = false) {
...
export async function notificarVendaConfirmada(
  ...
  const { porMlb, porSku } = ctx ?? await carregarProdutos(tenantId);
  const metaMargem = ctx ? ctx.metaMargem : await metaMargemAtual(tenantId);
  ...
  if (type === "sale_negative_margin") {
    const enviados = await enviarEPersistirEntrega(tenantId, eventId, type, payload);
  ...
  if (decisao.modo === "resumo_dispara") {
    ...
    const enviados = await enviarEPersistirEntrega(tenantId, eventId, type, payloadResumo, true);
    return { estado: "notificada", eventId, enviados };
  }
 ...
 */
export async function notificarMarco(tenantId: string, marco: { chave: string; titulo: string; corpo: string }): Promise<boolean> {
  const { created, eventId } = await createNotificationEventIdempotent(tenantId, {
    type: "system",
    severity: "success",
    entityType: "system",
